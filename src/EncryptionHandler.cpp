 ...
 */
EncryptionHandler::SK* EncryptionHandler::keyGen()
{
	return mSecretKey;
} //end of KeyGen()
 ...
 */
void EncryptionHandler::mapStringToElementFromGT(memberElement& ans,
		...
		CT& cipherText, StateMachine stateMachine)
{
	...
	for (int i = 1; i <= virusLength; i++)
	{
		...
		int indexAt_K_t = secretKey.getIndexInKeyComponentsByTransition(
				stateMachine.getCurrentStateID(), cipherText.mVirus.at(i - 1));
	...
	int indexAt_K_end =
			stateMachine.getIndexOfAcceptanceStateInTheAcceptanceStatesVector(
	...
	{
		cout
				<< "The given input didn't result in an acceptance state! decryption failed!";
		return;
	}

	mMapper->bilinearMap(temp1, cipherText.m_C_end1,
			secretKey.m_K_for_q_x[0][indexAt_K_end]); //map
	mMapper->invert(temp1, temp1);										//invert

	mMapper->bilinearMap(temp2, cipherText.m_C_end2,
			secretKey.m_K_for_q_x[1][indexAt_K_end]); //map
 ...
 */
BilinearMappingHandler* EncryptionHandler::getBilinearMappingHandler()
{
	return mMapper;
}
 ...
 * the SK Constructor
 * Logic:
 * Client - Allocated the needed memory and performs the required calculations.
 * Server/CA - Allocates memory and doesn't perform any calculation. All required parameters will be sent
 * by the client via a socket.
 */
EncryptionHandler::SK::SK(BilinearMappingHandler* mapper, StateMachine* M,
		MSK* msk, bool isClient)
{
	...
	//allocate and init all variables needed for intermediate calculations
	if (isClient)
	{
		...
		//allocate and init the vars for the calculation's final result:
		...
		const std::vector<Transition3Tuple>* T =
				M->translateStateMachineToTriples(); //T is a transitions vector
		int x_id, y_id;
		unsigned char sigma;
		...
		for (unsigned int i = 0; i < F->size(); i++)
		{
			...
			mMapper->power_Zn(m_K_for_q_x[1][i], mMasterKey->g,
					m_R_end_ExpSet[i]);		//K_endx2= g^(R_end_x)
			////done with K_end_x2
		}	//for
 ...
 */
int EncryptionHandler::SK::getIndexInKeyComponentsByTransition(int x,
		unsigned char sigma)
{
 ...
 */
void EncryptionHandler::CT::get_C_i_2(memberElement& ans, int i,
		unsigned char letter, bool isUsedWithPartialCT)
{
